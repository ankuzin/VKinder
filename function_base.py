...
def create_table():  # создание базы и таблиц
...
def zapolnenie_table1(id, city, relation, bdate):  # Заполнение таблицы "Информация о пользователе"
    connection = sql.connect('VKinder.db')
    with connection:
        cur = connection.cursor()
...
def zapolnenie_table2(id_users, link, foto1, foto2, foto3):  # Заполнение таблицы "Результаты поиска"
    connection = sql.connect('VKinder.db')
    with connection:
        cur = connection.cursor()
        cur.execute(
...
def delete_table1():  # удаление информации из таблицы "Информация о пользователе"
    connection = sql.connect('VKinder.db')
    with connection:
        cur = connection.cursor()
...
def delete_table2():  # удаление информации из таблицы "Результаты поиска"
    connection = sql.connect('VKinder.db')
    with connection:
        cur = connection.cursor()
